...
class Cluster
	...
	def is_in_cluster(poi)
		word_diff_with_most_common = get_name_difference(@most_common['name'], poi['name'])
		if word_diff_with_most_common > @@SAME_CLUSTER_DIFF_TOLERANCE
			# if this name isn't even remotely close to the name that represents the group, we assume that this doesn't belong here
			return false
		end
		
		# see if this is a good match for the name representing this group
		...
		for p in @pois
			if get_name_difference(p, poi['name']) <= SAME_WORD_DIFF_TOLERANCE
				return true
			end
		end
